...
- 在内存中生成一个**代表这个类的 `java. lang. Class` 对象**
- 懒加载: 类只有在被使用到的时候才会被加载
...
- 使用 C & C++ 编写, Java 代码无法直接访问
- 用与加载 Java 的核心类库, 提供 JVM 自身运行时所需的类, 负责加载 `JAVAHOME/jre/lib/rt.jar` 包下的 Java 运行时环境
### 扩展类加载器 (Extension ClassLoader)
- 上级为 BootStrap
- 负责加载 `JAVAHOME/jre/lib/ext` 包下的类
### 系统类加载器 (App ClassLoader)
- 加载 classpath 路径下的类, 也是程序中默认的类加载器, 一般来说, Java 应用的类都是由它来完成加载

## 双亲委派机制

### 简介
> [!info]
> 类加载器在接收到类加载请求时, 并不会直接加载这个类, 而是将这个加载的请求依次向上传递, 当父类加载器返回无法加载这个类的信息时, 这个子类加载器才会尝试自己去加载, 这个过程就是双亲委派机制
